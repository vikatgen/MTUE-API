  ...
  .then(data => {
    ...
    submitBtn.addEventListener("click", event => {
      ...
      if (selectedKey && hours) {
        ...
        rowDiv.classList.add("list-item", "remove-animation");
        rowDiv.innerHTML = `
            <div class="list-name-attribute">${
              occupations[selectedKey].name
            }</div>
            <div>${salary.toFixed(2).replace(",", ".") + " €"}</div>
            <div class="list-item-delete" data-salary-float="${salary}">X</div>
        `;
        rowDiv
          .querySelector(".list-item-delete")
          .addEventListener("click", event => {
            const currentRow = event.currentTarget.parentNode;
            const salary = event.currentTarget.getAttribute(
              "data-salary-float"
            );
            total -= salary;
